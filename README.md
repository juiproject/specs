...
├── README.md
├── src/
│   ├── req                ← CLI for requirements management (bash + sqlite3)
│   ├── trace              ← CLI for requirements traceability (bash + grep)
│   └── schema.sql         ← SQLite database schema
└── claude/
    └── skills/
        ├── req/           ← Skill for querying and managing requirements
        ├── spec-writer/   ← Skill for extracting and writing requirements
        └── trace/         ← Skill for code-to-requirements traceability
```

## `src/req`

The main CLI. A single bash script that manages a local SQLite database of requirements. Supports adding (interactive or YAML via stdin), listing, filtering, editing, updating (via YAML round-trip), tagging, domain assignment, approval workflows, exporting, snapshotting, and restoring requirements. Requirements are identified by display IDs (e.g. `AUTH-001`, `BIZ-003`) and organised into modules and categories.

Key capabilities beyond basic CRUD:
...
The SQLite schema for the requirements database. Used by `req init` to create or migrate the database. Defines tables for requirements, acceptance criteria (with an approved mirror for the approval workflow), tags, dependencies, domains, and modules.
...
A traceability CLI that scans Java source files for `@requirements` javadoc tags and cross-references them with the requirements database. Supports scanning, mapping code to requirements, coverage reporting, and orphan detection. Uses only bash and grep (plus sqlite3 for database commands).

## `claude/skills/trace/`

A Claude Code skill for inspecting and managing traceability between Java source code and requirements. When installed, Claude can mark requirements on classes and tests, verify code against acceptance criteria, and generate coverage reports - all driven by the `@requirements` javadoc tag convention.
...
cp src/trace specs/trace
cp src/schema.sql specs/schema.sql
chmod +x specs/req specs/trace
```
...
cp -r claude/skills/trace .claude/skills/trace
```
...
| Requirements Trace | Inspecting, verifying, and managing @requirements traceability between code and requirements | `.claude/skills/trace/SKILL.md` |
```
...
# Domains
specs/req domain add email-svc "Email Service" "Delivery and templating"
specs/req domain AUTH-001 add email-svc
specs/req list --domain email-svc

# Approval workflow
specs/req approve AUTH-001     # proposed → approved
specs/req diff AUTH-001        # show changes after editing an approved requirement
specs/req revert AUTH-001      # discard revisions, restore approved state
specs/req withdraw AUTH-001    # approved → proposed
...
Add `@requirements` tags to Java source to link code to requirements:
...
@Test
public void testLoginRejectsInvalidCredentials() {
```

Then use `specs/trace` to inspect and report:

```bash
# Scan the codebase for all @requirements tags
specs/trace scan

# Show all code linked to a specific requirement
specs/trace map --req AUTH-001

# Check which requirements have linked code and tests
specs/trace coverage

# Find tags that reference non-existent requirements
specs/trace orphans

# Combined coverage + orphans report
specs/trace summary
```
